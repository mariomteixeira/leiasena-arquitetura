...
export default function Header() {
    ...
    return (
        <>
            ...
            {open && (
                <div className="ac-menu fixed inset-0 z-[60] bg-cream md:hidden" role="dialog" aria-modal="true">
                    <div className="flex h-16 items-center justify-between px-5">
                        <span className="font-serif text-[1.45rem] italic leading-none text-navy">
                            {brand.shortName}
                        </span>
                        <button
                            type="button"
                    ...
                    <nav className="flex flex-col px-5 pt-6" aria-label="Principal">
                        {copy.sections.map((s, i) => (
                            <a
                                ...
                                className="ac-menu-item ac-hair-soft py-5 font-serif text-[2.35rem] leading-none text-navy first:border-t-0"
                            >
                                {s.label}
                            </a>
                        ))}
                    ...
                    <div
                        className="ac-menu-item absolute inset-x-5 bottom-9 flex flex-col gap-2 font-mono text-[0.72rem] uppercase tracking-[0.16em] text-navy/70"
                        style={{ ["--d" as string]: "420ms" }}
                    >
                        <a href={brand.whatsapp.href} target="_blank" rel="noreferrer" className="ac-link w-fit">
                            {brand.whatsapp.display}
                        </a>
                        <a href={brand.instagram.href} target="_blank" rel="noreferrer" className="ac-link w-fit">
